// $Id: findent.cpp 84 2015-07-30 21:36:07Z willem_vermin $
#include <cstdio>
#include <iostream>
#include <stack>
#include <queue>
...
void get_full_statement()
{
   ...
   while(1)
   {
	 ...
	 if (reading_from_tty && s == ".")
      ...
      if(in_preprocessor || (firstchar(s) == '#'))
      {
	 D(O("preproc");O(s););
	 if(handle_preproc(s))
	    in_preprocessor = 1;
...
bool handle_preproc(string s)
   ...
   // return 1: more preprocessor lines are to expected
...
void output_line()
{
   ...
   if (input_format == FREE)
   {
	 ...
	 if (firstline != "" || lines.size() > 1)
	 {
	    ...
	    if (l<0)
	       l=0;
	    if (firstchar(firstline) != '#')  // todo: this cannot happen
	    {
	       cout << string(l,' '); 
	 ...
	 D(O("output firstline");O(firstline););
      ...
      // sometimes, there are preprocessor statements within a continuation ...
         if (firstchar(lines.front()) == '#')
	 {
	    handle_pre(lines.front());
	    char lchar = lastchar(lines.front());
	    cout << lines.front() << endline;
	    lines.pop_front();
	    while(!lines.empty())
	    {
